...
FUNCTION /DMO/FLIGHT_TRAVEL_CALC_PRI_10.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
*"     REFERENCE(IT_FLIGHT) TYPE  /DMO/IF_FLIGHT_LEGACY10=>TT_FLIGHT
*"       OPTIONAL
*"----------------------------------------------------------------------
  TYPES BEGIN OF ty_seats_occupied.
  INCLUDE TYPE /dmo/flight10.
  TYPES   distance       TYPE /dmo/connecti_10-distance.
  TYPES   distance_unit  TYPE /dmo/connecti_10-distance_unit.
  ...
  IF it_flight IS SUPPLIED.
    SELECT FROM /dmo/booking10 AS booking
      JOIN     /dmo/connecti_10 AS connection
                 ON  booking~carrier_id    = connection~carrier_id
                 AND booking~connection_id = connection~connection_id
      JOIN     /dmo/flight10 AS flight
