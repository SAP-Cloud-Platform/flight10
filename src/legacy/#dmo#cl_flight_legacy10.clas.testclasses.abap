...
CLASS /dmo/cl_flight_legacy10 DEFINITION LOCAL FRIENDS ltcl_lock_travel.
...
CLASS ltc_travel DEFINITION FOR TESTING DURATION SHORT RISK LEVEL HARMLESS.
  PROTECTED SECTION.
    ...
    CONSTANTS mc_msgid                     TYPE symsgid   VALUE '/DMO/CM_FLIGHT_LE_10'.
...
*   CLASS-DATA mr_test_environment         TYPE REF TO if_osql_test_environment.
    ...
    CLASS-DATA gr_cut                      TYPE REF TO /dmo/cl_flight_legacy10.

    CLASS-DATA gv_agency_id_1              TYPE /dmo/agency_id10.
    CLASS-DATA gv_agency_id_2              TYPE /dmo/agency_id10.
    CLASS-DATA gv_agency_id_unknown        TYPE /dmo/agency_id10.

    CLASS-DATA gv_customer_id_1            TYPE /dmo/customer_id10.
    CLASS-DATA gv_customer_id_2            TYPE /dmo/customer_id10.
    CLASS-DATA gv_customer_id_unknown      TYPE /dmo/customer_id10.

    METHODS _create_travel IMPORTING is_travel        TYPE /dmo/if_flight_legacy10=>ts_travel_in
                                     iv_save          TYPE abap_bool DEFAULT abap_true
                           RETURNING VALUE(rs_travel) TYPE /dmo/travel10.
    METHODS _delete_existing_travel IMPORTING iv_travel_id TYPE /dmo/travel_id10.
...
CLASS /dmo/cl_flight_legacy10 DEFINITION LOCAL FRIENDS ltc_travel.

CLASS ltc_travel IMPLEMENTATION.
  METHOD class_setup.
    IF mc_use_sql_doubles = abap_true ##BOOL_OK.
*     mr_test_environment = cl_osql_test_environment=>create( i_dependency_list = VALUE #( ( '/DMO/TRAVEL10' ) ( '/DMO/BOOKING10' ) ( '/DMO/BOOK_SUP_10' )
*                                                                                          ( '/DMO/AGENCY10' ) ( '/DMO/CUSTOMER10' ) ( '/DMO/FLIGHT10' ) ( '/DMO/SUPPLEME_10' ) ) ).
*     mr_test_environment->clear_doubles( ).
*     gv_agency_id_1 = '42'.
*     gv_agency_id_2 = '43'.
*     DATA lt_agency TYPE STANDARD TABLE OF /dmo/agency10.
*     lt_agency = VALUE #( ( agency_id = gv_agency_id_1 ) ( agency_id = gv_agency_id_2 ) ).
*     mr_test_environment->insert_test_data( lt_agency ).
*
*     gv_customer_id_1 = '42'.
*     gv_customer_id_2 = '43'.
*     DATA lt_customer  TYPE STANDARD TABLE OF /dmo/customer10.
...
*     gv_customer_id_unknown = '99'.
    ELSE.
      DATA lt_agency_id TYPE SORTED TABLE OF /dmo/agency_id10     WITH UNIQUE KEY table_line.
      SELECT DISTINCT agency_id FROM /dmo/agency10     ORDER BY agency_id   DESCENDING INTO TABLE @lt_agency_id .

      DATA lt_customer_id TYPE SORTED TABLE OF /dmo/customer_id10 WITH UNIQUE KEY table_line.
      SELECT DISTINCT customer_id FROM /dmo/customer10 ORDER BY customer_id DESCENDING INTO TABLE @lt_customer_id .
  ...
  METHOD setup.
    SELECT COUNT( * ) FROM /dmo/travel10     INTO @mv_travel_count.
    SELECT COUNT( * ) FROM /dmo/booking10    INTO @mv_booking_count.
    SELECT COUNT( * ) FROM /dmo/book_sup_10 INTO @mv_booking_supplement_count.
  ENDMETHOD.


  METHOD teardown.
    " Ensure proper cleanup of each individual test method
    SELECT COUNT( * ) FROM /dmo/travel10 INTO @DATA(lv_travel_count).
    cl_abap_unit_assert=>assert_equals( act = lv_travel_count  exp = mv_travel_count ).
    SELECT COUNT( * ) FROM /dmo/booking10 INTO @DATA(lv_booking_count).
    cl_abap_unit_assert=>assert_equals( act = lv_booking_count  exp = mv_booking_count ).
    SELECT COUNT( * ) FROM /dmo/book_sup_10 INTO @DATA(lv_booking_supplement_count).
...
*     mr_test_environment->destroy( ).
  ...
  METHOD create.
    SELECT COUNT( * ) FROM /dmo/travel10 INTO @DATA(lv_count1).
    ...
    SELECT COUNT( * ) FROM /dmo/travel10 INTO @DATA(lv_count2).
    cl_abap_unit_assert=>assert_equals( msg = 'create should add a travel'  exp = 1  act = lv_count2 - lv_count1 ).

    SELECT * FROM /dmo/travel10 WHERE travel_id = @ls_travel_new-travel_id INTO TABLE @DATA(lt_travel).
    ...
    cl_abap_unit_assert=>assert_equals( msg = 'status' act = ls_travel-status exp = CONV /dmo/travel_status10( /dmo/if_flight_legacy10=>travel_status-new ) ).
  ...
  METHOD create_mutiple_calls.
    ...
    SELECT * FROM /dmo/travel10 WHERE travel_id = @ls_travel_1-travel_id OR travel_id = @ls_travel_2-travel_id INTO TABLE @DATA(lt_travel) ##SELECT_FAE_WITH_LOB[DESCRIPTION].
  ...
  METHOD c_agency_unknown.
    ...
    LOOP AT lt_messages INTO DATA(lr_message) ##INTO_OK.
      IF lr_message->t100key = /dmo/cx_flight_legacy10=>agency_unkown.
  ...
  METHOD c_customer_unknown.
    ...
    DATA lv_msg_found TYPE abap_bool.
    LOOP AT lt_messages INTO DATA(lr_message) ##INTO_OK.
      IF lr_message->t100key = /dmo/cx_flight_legacy10=>customer_unkown.
        lv_msg_found = abap_true.
      ENDIF.
    ENDLOOP.
    cl_abap_unit_assert=>assert_equals( msg = 'missing error' exp = abap_true act = lv_msg_found ).
  ...
  METHOD create_single.
    ...
    SELECT MAX( travel_id ) FROM /dmo/travel10 INTO @DATA(lv_travel_id_max).
    DATA lv_travel_id_1 TYPE /dmo/travel_id10.
    DATA lv_travel_id_2 TYPE /dmo/travel_id10.
    ...
    cl_abap_unit_assert=>assert_equals( act = _create_travel( VALUE #( agency_id = gv_agency_id_1  customer_id = gv_customer_id_2  begin_date = '20190101'  end_date = '20190201'
                                                                       status = /dmo/if_flight_legacy10=>travel_status-booked ) )-travel_id  exp = lv_travel_id_1 ).
    ...
    cl_abap_unit_assert=>assert_equals( act = _create_travel( VALUE #( agency_id = gv_agency_id_2  customer_id = gv_customer_id_1  begin_date = '20190101'  end_date = '20190201'
                                                                       status = /dmo/if_flight_legacy10=>travel_status-booked ) )-travel_id  exp = lv_travel_id_2 ).

    " Select and check the second travel
    DATA ls_travel_sel TYPE /dmo/travel10.
    SELECT SINGLE * FROM /dmo/travel10 WHERE travel_id = @lv_travel_id_2 INTO @ls_travel_sel.
    ...
    cl_abap_unit_assert=>assert_equals( act = ls_travel_sel-status         exp = CONV /dmo/travel_status10( /dmo/if_flight_legacy10=>travel_status-new ) )." Provided status overridden by determination
  ...
  METHOD delete_single.
    ...
    SELECT SINGLE description FROM /dmo/travel10 WHERE travel_id = @ls_travel_new-travel_id INTO @DATA(lv_description).
    cl_abap_unit_assert=>assert_subrc( ).
    ...
    CLEAR lv_db_exists.
    SELECT SINGLE FROM /dmo/travel10 FIELDS @abap_true WHERE travel_id = @ls_travel_new-travel_id INTO @lv_db_exists.
  ...
  METHOD update_single.
    ...
    DATA ls_travel_sel TYPE /dmo/travel10.
    SELECT SINGLE description FROM /dmo/travel10 WHERE travel_id = @ls_travel_new-travel_id INTO CORRESPONDING FIELDS OF @ls_travel_sel.
    cl_abap_unit_assert=>assert_subrc( ).
    ...
    CLEAR ls_travel_sel.
    SELECT SINGLE * FROM /dmo/travel10 WHERE travel_id = @ls_travel_new-travel_id INTO @ls_travel_sel.
    cl_abap_unit_assert=>assert_subrc( ).
  ...
  METHOD d_travel_id_unknown.
    ...
    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF /dmo/cx_flight_legacy10 ) ).
    DATA lx TYPE REF TO /dmo/cx_flight_legacy10.
    lx ?= lt_messages[ 1 ].
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = /dmo/cx_flight_legacy10=>travel_unknown-msgno ).
  ...
  METHOD delete_multiple.
    ...
    DATA lv_description TYPE /dmo/description10.

    SELECT SINGLE description FROM /dmo/travel10 WHERE travel_id = @ls_travel_new_1-travel_id INTO @lv_description.
    cl_abap_unit_assert=>assert_subrc( ).
    cl_abap_unit_assert=>assert_equals( act = lv_description  exp = ls_travel_new_1-description ).

    SELECT SINGLE description FROM /dmo/travel10 WHERE travel_id = @ls_travel_new_2-travel_id INTO @lv_description.
    cl_abap_unit_assert=>assert_subrc( ).
    cl_abap_unit_assert=>assert_equals( act = lv_description  exp = ls_travel_new_2-description ).

    SELECT SINGLE description FROM /dmo/travel10 WHERE travel_id = @ls_travel_new_3-travel_id INTO @lv_description.
    cl_abap_unit_assert=>assert_subrc( ).
    ...
    CLEAR lv_db_exists.
    SELECT SINGLE FROM /dmo/travel10 FIELDS @abap_true WHERE travel_id IN ( @ls_travel_new_1-travel_id, @ls_travel_new_2-travel_id, @ls_travel_new_3-travel_id ) INTO @lv_db_exists.
    cl_abap_unit_assert=>assert_false( lv_db_exists ).
  ENDMETHOD.
  ...
  METHOD update_multiple.
    ...
    CLEAR ls_travel_sel.
    SELECT SINGLE agency_id, customer_id, description FROM /dmo/travel10 WHERE travel_id = @ls_travel_new_1-travel_id INTO CORRESPONDING FIELDS OF @ls_travel_sel.
    cl_abap_unit_assert=>assert_subrc( ).
    ...
    CLEAR ls_travel_sel.
    SELECT SINGLE agency_id, customer_id, description FROM /dmo/travel10 WHERE travel_id = @ls_travel_new_2-travel_id INTO CORRESPONDING FIELDS OF @ls_travel_sel.
    cl_abap_unit_assert=>assert_subrc( ).
    ...
    CLEAR ls_travel_sel.
    SELECT SINGLE agency_id, customer_id, description FROM /dmo/travel10 WHERE travel_id = @ls_travel_new_3-travel_id INTO CORRESPONDING FIELDS OF @ls_travel_sel.
    cl_abap_unit_assert=>assert_subrc( ).
    cl_abap_unit_assert=>assert_equals( act = ls_travel_sel-agency_id    exp = gv_agency_id_2   ).
  ...
  METHOD update_twice.
    ...
    CLEAR ls_travel_sel.
    SELECT SINGLE agency_id, customer_id, description FROM /dmo/travel10 WHERE travel_id = @ls_travel_new-travel_id INTO CORRESPONDING FIELDS OF @ls_travel_sel.
    cl_abap_unit_assert=>assert_subrc( ).
    cl_abap_unit_assert=>assert_equals( act = ls_travel_sel-agency_id    exp = gv_agency_id_2 ).
    cl_abap_unit_assert=>assert_equals( act = ls_travel_sel-customer_id  exp = ls_travel_new-customer_id ).
    cl_abap_unit_assert=>assert_equals( act = ls_travel_sel-description  exp = 'My_New_Text' ).
  ...
  METHOD u_agency_unknown.
    ...
    lx ?= lt_messages[ 1 ].
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = /dmo/cx_flight_legacy10=>agency_unkown-msgno ).
  ...
  METHOD u_customer_unknown.
    ...
    lx ?= lt_messages[ 1 ].
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = /dmo/cx_flight_legacy10=>customer_unkown-msgno ).
  ...
  METHOD d_travel_id_initial.
    ...
    lx ?= lt_messages[ 1 ].
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = /dmo/cx_flight_legacy10=>travel_no_key-msgno ).
  ENDMETHOD.
  ...
  METHOD act_set_status_to_booked.
    ...
    DATA ls_travel_sel TYPE /dmo/travel10.
    SELECT SINGLE status FROM /dmo/travel10 WHERE travel_id = @ls_travel_new-travel_id INTO CORRESPONDING FIELDS OF @ls_travel_sel.
    cl_abap_unit_assert=>assert_subrc( ).
    cl_abap_unit_assert=>assert_equals( act = ls_travel_sel-status  exp = CONV /dmo/travel_status10( /dmo/if_flight_legacy10=>travel_status-new ) ).
    ...
    CLEAR ls_travel_sel.
    SELECT SINGLE status, createdat, lastchangedby, lastchangedat FROM /dmo/travel10 WHERE travel_id = @ls_travel_new-travel_id INTO CORRESPONDING FIELDS OF @ls_travel_sel.
    cl_abap_unit_assert=>assert_subrc( ).
    cl_abap_unit_assert=>assert_equals( act = ls_travel_sel-status  exp = CONV /dmo/travel_status10( /dmo/if_flight_legacy10=>travel_status-booked ) ).
    cl_abap_unit_assert=>assert_equals( act = ls_travel_sel-lastchangedby  exp = sy-uname ).
    cl_abap_unit_assert=>assert_differs( act = ls_travel_sel-createdat  exp = ls_travel_sel-lastchangedat ).
    DATA(lv_diff) = cl_abap_tstmp=>subtract( tstmp1 = ls_travel_sel-lastchangedat  tstmp2 = lv_timestampl ).
    cl_abap_unit_assert=>assert_true( xsdbool( 0 < lv_diff AND lv_diff < 1 ) ).
  ...
  METHOD c_dates_invalid.
    ...
    lx ?= lt_messages[ 1 ].
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = /dmo/cx_flight_legacy10=>no_begin_date-msgno ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr1  exp = 'MV_TRAVEL_ID' ).
    ...
    lx ?= lt_messages[ 1 ].
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = /dmo/cx_flight_legacy10=>no_end_date-msgno ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr1  exp = 'MV_TRAVEL_ID' ).
    ...
    lx ?= lt_messages[ 1 ].
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = /dmo/cx_flight_legacy10=>end_date_before_begin_date-msgno ).
  ...
  METHOD create_update_in_one_luw.
    ...
    SELECT SINGLE agency_id, description FROM /dmo/travel10 WHERE travel_id = @ls_travel-travel_id INTO ( @DATA(lv_agency_id), @DATA(lv_description) ).
    cl_abap_unit_assert=>assert_subrc( ).
  ...
  METHOD create_delete_in_one_luw.
    ...
    CLEAR lv_db_exits.
    SELECT SINGLE FROM /dmo/travel10 FIELDS @abap_true WHERE travel_id = @ls_travel-travel_id INTO @lv_db_exits.
  ...
  METHOD update_delete_in_one_luw.
    ...
    CLEAR lv_db_exits.
    SELECT SINGLE FROM /dmo/travel10 FIELDS @abap_true WHERE travel_id = @lv_travel_id INTO @lv_db_exits.
    cl_abap_unit_assert=>assert_false( lv_db_exits ).
  ENDMETHOD.
  ...
  METHOD u_no_control.
    ...
    lx ?= lt_messages[ 1 ].
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = /dmo/cx_flight_legacy10=>travel_no_control-msgno ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr1  exp = 'MV_TRAVEL_ID' ).
    cl_abap_unit_assert=>assert_equals( act = lx->mv_travel_id                   exp = ls_travel-travel_id ).
  ...
  METHOD u_status_invalid.
    CONSTANTS lc_status_invalid TYPE /dmo/travel_status10 VALUE 'Z'.
    ...
    lx ?= lt_messages[ 1 ].
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = /dmo/cx_flight_legacy10=>travel_status_invalid-msgno ).
...
CLASS ltc_booking DEFINITION FOR TESTING DURATION SHORT RISK LEVEL HARMLESS INHERITING FROM ltc_travel.
  PROTECTED SECTION.
    TYPES: BEGIN OF ts_flight,
             ...
             currency_code TYPE /dmo/currency_code10,
           END OF ts_flight.

    CLASS-DATA gs_flight_1 TYPE ts_flight.
    CLASS-DATA gs_flight_2 TYPE ts_flight.

    CLASS-DATA gv_carrier_id_unknown       TYPE /dmo/carrier_id10.

    CLASS-DATA gv_currency_code_unknown    TYPE /dmo/currency_code10.

    CLASS-DATA gv_booking_date             TYPE /dmo/booking_date10.

    DATA mv_travel_id         TYPE /dmo/travel_id10.
    DATA mv_travel_id_unknown TYPE /dmo/travel_id10.

    METHODS _create_booking IMPORTING iv_travel_id      TYPE /dmo/travel_id10
                                      is_booking        TYPE /dmo/if_flight_legacy10=>ts_booking_in
                                      iv_save           TYPE abap_bool DEFAULT abap_true
                            RETURNING VALUE(rs_booking) TYPE /dmo/booking10.
    METHODS _delete_existing_booking IMPORTING iv_travel_id  TYPE /dmo/travel_id10
...
CLASS /dmo/cl_flight_legacy10 DEFINITION LOCAL FRIENDS ltc_booking.
...
*
*     DATA lt_flight TYPE STANDARD TABLE OF /dmo/flight10.
...
*     gv_currency_code_unknown = 'XYZ'.
    ELSE.
      ...
      SELECT SINGLE carrier_id, connection_id, flight_date, price, currency_code FROM /dmo/flight10
        INTO CORRESPONDING FIELDS OF @gs_flight_1 ##WARN_OK.
      IF sy-subrc <> 0.
        cl_abap_unit_assert=>abort( |No flight data!| ).
      ENDIF.
      SELECT SINGLE carrier_id, connection_id, flight_date, price, currency_code FROM /dmo/flight10 WHERE carrier_id <> @gs_flight_1-carrier_id AND connection_id <> @gs_flight_1-connection_id
      ...
      SELECT SINGLE carrier_id FROM /dmo/carrier10 WHERE carrier_id = @gv_carrier_id_unknown INTO @DATA(lv_dummy_carrier_id).
      IF sy-subrc = 0.
  ...
  METHOD setup.
    ...
    DO.
      ...
      SELECT SINGLE travel_id FROM /dmo/travel10 WHERE travel_id = @mv_travel_id_unknown INTO @DATA(lv_dummy_travel_id).
      IF sy-subrc <> 0.
        EXIT.
      ENDIF.
    ENDDO.
  ...
  METHOD c_u_d_single.
    ...
    SELECT SINGLE * FROM /dmo/booking10 WHERE travel_id = @mv_travel_id AND booking_id = @ls_booking-booking_id INTO @DATA(ls_booking_sel).
    cl_abap_unit_assert=>assert_subrc( ).
    ...
    gr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = mv_travel_id )
                                     ...
                                     it_bookingx = VALUE #( ( booking_id = ls_booking-booking_id  action_code = /dmo/if_flight_legacy10=>action_code-update  booking_date = abap_true ) )
                           IMPORTING et_messages = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_initial( lt_messages ).
    gr_cut->save( ).

    CLEAR ls_booking_sel.
    SELECT SINGLE * FROM /dmo/booking10 WHERE travel_id = @mv_travel_id AND booking_id = @ls_booking-booking_id INTO @ls_booking_sel.
    cl_abap_unit_assert=>assert_subrc( ).
  ...
  METHOD cud_single.
    ...
    SELECT SINGLE lastchangedat FROM /dmo/travel10 WHERE travel_id = @mv_travel_id INTO @DATA(lv_lastchangedat_1).
    ...
    DATA lv_booking_date TYPE /dmo/booking_date10.
    ...
    gr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = mv_travel_id )
                                     ...
                                     it_bookingx = VALUE #( ( booking_id = ls_booking-booking_id  action_code = /dmo/if_flight_legacy10=>action_code-delete ) )
                           IMPORTING et_messages = lt_messages ).
    cl_abap_unit_assert=>assert_initial( lt_messages ).
    ...
    SELECT SINGLE lastchangedby, lastchangedat FROM /dmo/travel10 WHERE travel_id = @mv_travel_id INTO ( @DATA(lv_lastchangedby), @DATA(lv_lastchangedat_2) ).
    ...
    CLEAR lv_db_exists.
    SELECT SINGLE FROM /dmo/booking10 FIELDS @abap_true WHERE travel_id = @mv_travel_id AND booking_id = @ls_booking-booking_id INTO @lv_db_exists.
    cl_abap_unit_assert=>assert_false( lv_db_exists ).
  ENDMETHOD.
  ...
  METHOD c_uu_d_single.
    ...
    SELECT SINGLE customer_id FROM /dmo/booking10 WHERE travel_id = @mv_travel_id AND booking_id = @ls_booking-booking_id INTO @DATA(lv_customer_id).
    cl_abap_unit_assert=>assert_subrc( ).
    ...
    gr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = mv_travel_id )
                                     ...
                                     it_bookingx = VALUE #( ( booking_id = ls_booking-booking_id  action_code = /dmo/if_flight_legacy10=>action_code-update  customer_id = abap_true ) )
                           IMPORTING et_messages = lt_messages ).
    cl_abap_unit_assert=>assert_initial( lt_messages ).
    gr_cut->save( ).

    DATA ls_booking_sel TYPE /dmo/booking10.
    SELECT SINGLE booking_date, customer_id FROM /dmo/booking10 WHERE travel_id = @mv_travel_id AND booking_id = @ls_booking-booking_id INTO CORRESPONDING FIELDS OF @ls_booking_sel.
    cl_abap_unit_assert=>assert_subrc( ).
    cl_abap_unit_assert=>assert_equals( act = ls_booking_sel-booking_date  exp = lv_booking_date ).
  ...
  METHOD c_travel_id_unknown.
    gr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = mv_travel_id_unknown )
                                     ...
                                     it_bookingx = VALUE #( ( booking_id = '10'  action_code = /dmo/if_flight_legacy10=>action_code-create ) )
  ...
  METHOD u_travel_id_unknown.
    gr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = mv_travel_id_unknown )
                                     ...
                                     it_bookingx = VALUE #( ( booking_id = '2'  action_code = /dmo/if_flight_legacy10=>action_code-update  booking_date = abap_true ) )
                           IMPORTING et_messages = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF /dmo/cx_flight_legacy10 ) ).
    DATA lx TYPE REF TO /dmo/cx_flight_legacy10.
  ...
  METHOD d_travel_id_unknown.
    gr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = mv_travel_id_unknown )
                                     ...
                                     it_bookingx = VALUE #( ( booking_id = '1'  action_code = /dmo/if_flight_legacy10=>action_code-delete ) )
                           IMPORTING et_messages = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF /dmo/cx_flight_legacy10 ) ).
    DATA lx TYPE REF TO /dmo/cx_flight_legacy10.
  ...
  METHOD u_booking_id_unknown.
    gr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = mv_travel_id )
                                     ...
                                     it_bookingx = VALUE #( ( booking_id = '10'  action_code = /dmo/if_flight_legacy10=>action_code-update  booking_date = abap_true  ) )
                           IMPORTING et_messages = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF /dmo/cx_flight_legacy10 ) ).
    DATA lx TYPE REF TO /dmo/cx_flight_legacy10.
    lx ?= lt_messages[ 1 ].
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = /dmo/cx_flight_legacy10=>booking_unknown-msgno ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr1  exp = 'MV_TRAVEL_ID' ).
  ...
  METHOD d_booking_id_unknown.
    gr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = mv_travel_id )
                                     ...
                                     it_bookingx = VALUE #( ( booking_id = '10'  action_code = /dmo/if_flight_legacy10=>action_code-delete ) )
                           IMPORTING et_messages = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF /dmo/cx_flight_legacy10 ) ).
    DATA lx TYPE REF TO /dmo/cx_flight_legacy10.
  ...
  METHOD u_booking_id_initial.
    gr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = mv_travel_id )
                                     ...
                                     it_bookingx = VALUE #( ( booking_id = '0'  action_code = /dmo/if_flight_legacy10=>action_code-update  booking_date = abap_true ) )
                           IMPORTING et_messages = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF /dmo/cx_flight_legacy10 ) ).
    DATA lx TYPE REF TO /dmo/cx_flight_legacy10.
    lx ?= lt_messages[ 1 ].
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = /dmo/cx_flight_legacy10=>booking_no_key-msgno ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr1  exp = 'MV_TRAVEL_ID' ).
    cl_abap_unit_assert=>assert_equals( act = lx->mv_travel_id                   exp = mv_travel_id ).
  ENDMETHOD.
  ...
  METHOD d_booking_id_initial.
    gr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = mv_travel_id )
                                     ...
                                     it_bookingx = VALUE #( ( booking_id = '0'  action_code = /dmo/if_flight_legacy10=>action_code-delete ) )
                           IMPORTING et_messages = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF /dmo/cx_flight_legacy10 ) ).
    DATA lx TYPE REF TO /dmo/cx_flight_legacy10.
  ...
  METHOD c_u_d_travel_w_bookings.
    ...
    SELECT COUNT( * ) FROM /dmo/booking10 WHERE travel_id = @ls_travel_1-travel_id INTO @DATA(lv_count).
    ...
    gr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = ls_booking_1-travel_id )
                                     ...
                                     it_bookingx = VALUE #( ( booking_id = ls_booking_1-booking_id  action_code = /dmo/if_flight_legacy10=>action_code-update  customer_id = abap_true ) )
                           IMPORTING et_messages = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_initial( lt_messages ).

    gr_cut->save( ).

    SELECT SINGLE customer_id FROM /dmo/booking10 WHERE travel_id = @ls_booking_1-travel_id AND booking_id = @ls_booking_1-booking_id INTO @DATA(lv_customer_id).
    cl_abap_unit_assert=>assert_subrc( ).
    cl_abap_unit_assert=>assert_equals( act = lv_customer_id  exp = gv_customer_id_2 ).

    SELECT SINGLE customer_id FROM /dmo/booking10 WHERE travel_id = @ls_booking_2-travel_id AND booking_id = @ls_booking_2-booking_id INTO @lv_customer_id.
    cl_abap_unit_assert=>assert_subrc( ).
    ...
    SELECT travel_id, booking_id FROM /dmo/booking10 WHERE travel_id = @ls_travel_1-travel_id INTO TABLE @DATA(lt_booking_key).
    ...
    CLEAR lv_db_exists.
    SELECT SINGLE FROM /dmo/booking10 FIELDS @abap_true WHERE travel_id = @ls_travel_1-travel_id INTO @lv_db_exists.
    cl_abap_unit_assert=>assert_false( lv_db_exists ).
  ENDMETHOD.
  ...
  METHOD c_customer_unknown.
    gr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = mv_travel_id )
                                     ...
                                     it_bookingx = VALUE #( ( booking_id  = '10'
                                                              action_code = /dmo/if_flight_legacy10=>action_code-create ) )
                           IMPORTING et_booking  = DATA(lt_booking)
                                     et_messages = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_initial( lt_booking ).
    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
  ...
  METHOD c_booking_date_past.
    ...
    lx ?= lt_messages[ 1 ].
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = /dmo/cx_flight_legacy10=>booking_booking_date_invalid-msgno ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr1  exp = 'MV_TRAVEL_ID' ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr2  exp = 'MV_BOOKING_ID' ).
  ...
  METHOD u_booking_date_initial.
    ...
    SELECT SINGLE booking_date FROM /dmo/booking10 WHERE travel_id = @mv_travel_id AND booking_id = @ls_booking-booking_id INTO @DATA(lv_booking_date).
    cl_abap_unit_assert=>assert_subrc( ).
  ...
  METHOD c_flight_invalid.
    ...
    lx ?= lt_messages[ 1 ].
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = /dmo/cx_flight_legacy10=>flight_unknown-msgno ).
  ...
  METHOD u_flight.
    ...
    gr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = mv_travel_id )
                                     ...
                                     it_bookingx = VALUE #( ( booking_id = ls_booking-booking_id  action_code = /dmo/if_flight_legacy10=>action_code-update  flight_date = abap_true ) )
                           IMPORTING et_messages = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF /dmo/cx_flight_legacy10 ) ).
    DATA lx TYPE REF TO /dmo/cx_flight_legacy10.
    lx ?= lt_messages[ 1 ].
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = /dmo/cx_flight_legacy10=>booking_flight_u-msgno ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr1  exp = 'MV_TRAVEL_ID' ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr2  exp = 'MV_BOOKING_ID' ).
    cl_abap_unit_assert=>assert_equals( act = lx->mv_travel_id                   exp = mv_travel_id ).
    cl_abap_unit_assert=>assert_equals( act = lx->mv_booking_id                  exp = '10' ).
    ...
    SELECT SINGLE flight_date FROM /dmo/booking10 WHERE travel_id = @mv_travel_id AND booking_id = @ls_booking-booking_id INTO @DATA(lv_flight_date).
    cl_abap_unit_assert=>assert_subrc( ).
  ...
  METHOD c_booking_d_travel_1_luw.
    ...
    DATA(ls_travel) = _create_travel( VALUE #( agency_id = gv_agency_id_1  customer_id = gv_customer_id_1  begin_date = '20190101'  end_date = '20190201' ) ).
    CLEAR lv_db_exists.
    SELECT SINGLE FROM /dmo/travel10 FIELDS @abap_true WHERE travel_id = @ls_travel-travel_id INTO @lv_db_exists.
    ...
    CLEAR lv_db_exists.
    SELECT SINGLE FROM /dmo/booking10 FIELDS @abap_true WHERE travel_id = @ls_travel-travel_id INTO @lv_db_exists.
    cl_abap_unit_assert=>assert_false( lv_db_exists ).
  ENDMETHOD.
  ...
  METHOD c_booking_id_exists.
    ...
    lx ?= lt_messages[ 1 ].
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = /dmo/cx_flight_legacy10=>booking_exists-msgno ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr1  exp = 'MV_TRAVEL_ID' ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr2  exp = 'MV_BOOKING_ID' ).
    cl_abap_unit_assert=>assert_equals( act = lx->mv_travel_id                   exp = mv_travel_id ).
    cl_abap_unit_assert=>assert_equals( act = lx->mv_booking_id                  exp = '10' ).
  ...
  METHOD c_price_currency.
    ...
    DATA lv_flight_price  TYPE /dmo/flight_price10.
    DATA lv_currency_code TYPE /dmo/currency_code10.
    ...
    DATA ls_booking_sel TYPE /dmo/booking10.
    SELECT SINGLE flight_price, currency_code FROM /dmo/booking10 WHERE travel_id = @ls_booking_1-travel_id AND booking_id = @ls_booking_1-booking_id INTO CORRESPONDING FIELDS OF @ls_booking_sel.
    cl_abap_unit_assert=>assert_subrc( ).
    ...
    CLEAR ls_booking_sel.
    SELECT SINGLE flight_price, currency_code FROM /dmo/booking10 WHERE travel_id = @ls_booking_1-travel_id AND booking_id = @ls_booking_2-booking_id INTO CORRESPONDING FIELDS OF @ls_booking_sel.
    cl_abap_unit_assert=>assert_subrc( ).
  ...
  METHOD u_price_currency.
    ...
    gr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = ls_booking-travel_id )
                                     ...
                                     it_bookingx = VALUE #( ( booking_id = ls_booking-booking_id  action_code = /dmo/if_flight_legacy10=>action_code-update  flight_price = abap_true ) )
                           IMPORTING et_messages = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF /dmo/cx_flight_legacy10 ) ).
    DATA lx TYPE REF TO /dmo/cx_flight_legacy10.
    lx ?= lt_messages[ 1 ].
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = /dmo/cx_flight_legacy10=>booking_price_currency_u-msgno ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr1  exp = 'MV_TRAVEL_ID' ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr2  exp = 'MV_BOOKING_ID' ).
    ...
    gr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = ls_booking-travel_id )
                                     ...
                                     it_bookingx = VALUE #( ( booking_id = ls_booking-booking_id  action_code = /dmo/if_flight_legacy10=>action_code-update  currency_code = abap_true ) )
                           IMPORTING et_messages = lt_messages ).
    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF /dmo/cx_flight_legacy10 ) ).
    lx ?= lt_messages[ 1 ].
  ...
  METHOD c_currency_code_unknown.
    ...
    lx ?= lt_messages[ 1 ].
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = /dmo/cx_flight_legacy10=>currency_unknown-msgno ).
  ...
  METHOD u_currency_code_unknown.
    ...
    gr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = ls_booking-travel_id )
                                     ...
                                     it_bookingx = VALUE #( ( booking_id = ls_booking-booking_id  action_code = /dmo/if_flight_legacy10=>action_code-update  flight_price = abap_true  currency_code = abap_true ) )
                           IMPORTING et_messages = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF /dmo/cx_flight_legacy10 ) ).
    DATA lx TYPE REF TO /dmo/cx_flight_legacy10.
  ...
  METHOD _create_booking.
    gr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = iv_travel_id )
                                     ...
                                     it_bookingx = VALUE #( ( booking_id = is_booking-booking_id  action_code = /dmo/if_flight_legacy10=>action_code-create ) )
                           IMPORTING et_booking  = DATA(lt_booking)
                                     et_messages = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_initial( lt_messages ).
  ...
  METHOD _delete_existing_booking.
    gr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = iv_travel_id )
                                     ...
                                     it_bookingx = VALUE #( ( booking_id = iv_booking_id  action_code = /dmo/if_flight_legacy10=>action_code-delete ) )
                           IMPORTING et_messages = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_initial( lt_messages ).
    gr_cut->save( ).
  ENDMETHOD.
...
CLASS ltc_booking_supplement DEFINITION FINAL FOR TESTING DURATION SHORT RISK LEVEL HARMLESS INHERITING FROM ltc_booking.
  PRIVATE SECTION.
    TYPES: BEGIN OF ts_supplement,
             supplement_id TYPE /dmo/supplement_id10,
             price         TYPE /dmo/supplement_price10,
             currency_code TYPE /dmo/currency_code10,
    ...
    CLASS-DATA gv_supplement_id_unknown      TYPE /dmo/supplement_id10.

    DATA mv_booking_id         TYPE /dmo/booking_id10.
    DATA mv_booking_id_unknown TYPE /dmo/booking_id10.
    ...
    METHODS _create_booking_suppl IMPORTING iv_travel_id                 TYPE /dmo/travel_id10
                                            is_booking_supplement        TYPE /dmo/if_flight_legacy10=>ts_booking_supplement_in
                                            iv_save                      TYPE abap_bool DEFAULT abap_true
                                  RETURNING VALUE(rs_booking_supplement) TYPE /dmo/book_sup_10.
ENDCLASS.

CLASS /dmo/cl_flight_legacy10 DEFINITION LOCAL FRIENDS ltc_booking_supplement.
...
*
*     DATA lt_supplement TYPE STANDARD TABLE OF /dmo/suppleme_10.
...
*     gv_supplement_id_unknown      = 'XX-999'.
    ELSE.
      ...
      SELECT SINGLE supplement_id, price, currency_code FROM /dmo/suppleme_10 INTO ( @gs_supplement_1-supplement_id, @gs_supplement_1-price, @gs_supplement_1-currency_code ) ##WARN_OK.
      IF sy-subrc <> 0.
        cl_abap_unit_assert=>abort( 'No supplement data!' ).
      ENDIF.

      " Select a different valid combination of Supplement ID, Price, Currency Code
      SELECT SINGLE supplement_id, price, currency_code FROM /dmo/suppleme_10 WHERE supplement_id <> @gs_supplement_1-supplement_id
      ...
      gv_supplement_id_unknown = 'XX-999'.
      SELECT SINGLE supplement_id FROM /dmo/suppleme_10 WHERE supplement_id = @gv_supplement_id_unknown INTO @DATA(lv_dummy_supplement_id).
      IF sy-subrc = 0.
  ...
  METHOD c_u_d_multiple.
    ...
    SELECT COUNT( * ) FROM /dmo/book_sup_10 WHERE travel_id = @ls_travel-travel_id INTO @DATA(lv_count).
    cl_abap_unit_assert=>assert_equals( act = lv_count  exp = 3 ).

    DATA ls_book_suppl_sel TYPE /dmo/book_sup_10.

    SELECT SINGLE price, currency_code FROM /dmo/book_sup_10 WHERE travel_id             = @ls_book_suppl_2_1-travel_id
    ...
    SELECT SINGLE price, currency_code FROM /dmo/book_sup_10 WHERE travel_id             = @ls_book_suppl_2_2-travel_id
    ...
    SELECT COUNT( * ) FROM /dmo/book_sup_10 WHERE travel_id = @ls_travel-travel_id INTO @lv_count.
    cl_abap_unit_assert=>assert_equals( act = lv_count  exp = 2 ).

    SELECT COUNT( * ) FROM /dmo/book_sup_10 WHERE travel_id = @ls_travel-travel_id AND booking_id = @ls_booking_2-booking_id INTO @lv_count.
    cl_abap_unit_assert=>assert_equals( act = lv_count  exp = 2 ).

    " 4. LUW: Delete the second booking supplement of the second booking - here check also travel admin field
    SELECT SINGLE lastchangedat FROM /dmo/travel10 WHERE travel_id = @ls_travel-travel_id INTO @DATA(lv_lastchangedat_1).
    cl_abap_unit_assert=>assert_subrc( ).

    CLEAR lv_db_exists.
    SELECT SINGLE FROM /dmo/book_sup_10 FIELDS @abap_true WHERE travel_id             = @ls_book_suppl_2_2-travel_id
                                                           AND booking_id            = @ls_book_suppl_2_2-booking_id
    ...
    gr_cut->update_travel( EXPORTING is_travel              = VALUE #( travel_id = ls_book_suppl_2_2-travel_id )
                                     ...
                                     it_booking_supplementx = VALUE #( ( booking_id            = ls_book_suppl_2_2-booking_id
                                                                         booking_supplement_id = ls_book_suppl_2_2-booking_supplement_id
                                                                         action_code = /dmo/if_flight_legacy10=>action_code-delete ) )
                           IMPORTING et_messages            = lt_messages ).
    cl_abap_unit_assert=>assert_initial( lt_messages ).
    gr_cut->save( ).
    ...
    SELECT SINGLE lastchangedat FROM /dmo/travel10 WHERE travel_id = @ls_travel-travel_id INTO @DATA(lv_lastchangedat_2).
    cl_abap_unit_assert=>assert_subrc( ).
    ...
    CLEAR lv_db_exists.
    SELECT SINGLE FROM /dmo/book_sup_10 FIELDS @abap_true WHERE travel_id = @ls_travel-travel_id INTO @lv_db_exists.
    cl_abap_unit_assert=>assert_false( lv_db_exists ).
  ENDMETHOD.
  ...
  METHOD u_travel_id_unknown.
    gr_cut->update_travel( EXPORTING is_travel              = VALUE #( travel_id = mv_travel_id_unknown )
                                     ...
                                     it_booking_supplementx = VALUE #( ( booking_id = '2'  action_code = /dmo/if_flight_legacy10=>action_code-update  price = abap_true ) )
                           IMPORTING et_messages            = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF /dmo/cx_flight_legacy10 ) ).
    DATA lx TYPE REF TO /dmo/cx_flight_legacy10.
  ...
  METHOD u_travel_id_initial.
    gr_cut->update_travel( EXPORTING is_travel              = VALUE #( )
                                     ...
                                     it_booking_supplementx = VALUE #( ( booking_id = '2'  booking_supplement_id = '1'  action_code = /dmo/if_flight_legacy10=>action_code-update  price = abap_true ) )
                           IMPORTING et_messages            = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF /dmo/cx_flight_legacy10 ) ).
    DATA lx TYPE REF TO /dmo/cx_flight_legacy10.
  ...
  METHOD d_travel_id_initial.
    gr_cut->update_travel( EXPORTING is_travel              = VALUE #( )
                                     ...
                                     it_booking_supplementx = VALUE #( ( booking_id = '1'  booking_supplement_id = '1'  action_code = /dmo/if_flight_legacy10=>action_code-delete ) )
                           IMPORTING et_messages            = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF /dmo/cx_flight_legacy10 ) ).
    DATA lx TYPE REF TO /dmo/cx_flight_legacy10.
  ...
  METHOD c_booking_id_initial.
    gr_cut->update_travel( EXPORTING is_travel              = VALUE #( travel_id = mv_travel_id )
                                     ...
                                     it_booking_supplementx = VALUE #( ( booking_supplement_id = '1'  action_code = /dmo/if_flight_legacy10=>action_code-create ) )
                           IMPORTING et_booking_supplement  = DATA(lt_booking_supplement)
                                     et_messages            = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_initial( lt_booking_supplement ).
    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
  ...
  METHOD u_booking_id_unknown.
    gr_cut->update_travel( EXPORTING is_travel              = VALUE #( travel_id = mv_travel_id )
                                     ...
                                     it_booking_supplementx = VALUE #( ( booking_id = mv_booking_id_unknown  booking_supplement_id = '1'
                                                                         action_code = /dmo/if_flight_legacy10=>action_code-update  price = abap_true ) )
                           IMPORTING et_messages            = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF /dmo/cx_flight_legacy10 ) ).
    DATA lx TYPE REF TO /dmo/cx_flight_legacy10.
  ...
  METHOD u_booking_id_initial.
    gr_cut->update_travel( EXPORTING is_travel              = VALUE #( travel_id = mv_travel_id )
                                     ...
                                     it_booking_supplementx = VALUE #( ( booking_supplement_id = '1'  action_code = /dmo/if_flight_legacy10=>action_code-update  price = abap_true ) )
                           IMPORTING et_messages            = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF /dmo/cx_flight_legacy10 ) ).
    DATA lx TYPE REF TO /dmo/cx_flight_legacy10.
  ...
  METHOD d_booking_id_unknown.
    gr_cut->update_travel( EXPORTING is_travel              = VALUE #( travel_id = mv_travel_id )
                                     ...
                                     it_booking_supplementx = VALUE #( ( booking_id = mv_booking_id_unknown  booking_supplement_id = '1'  action_code = /dmo/if_flight_legacy10=>action_code-delete ) )
                           IMPORTING et_messages            = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF /dmo/cx_flight_legacy10 ) ).
    DATA lx TYPE REF TO /dmo/cx_flight_legacy10.
  ...
  METHOD d_booking_id_initial.
    gr_cut->update_travel( EXPORTING is_travel              = VALUE #( travel_id = mv_travel_id )
                                     is_travelx             = VALUE #( travel_id = mv_travel_id )
                                     it_booking_supplement  = VALUE #( ( booking_supplement_id = '1' ) )
                                     it_booking_supplementx = VALUE #( ( booking_supplement_id = '1'  action_code = /dmo/if_flight_legacy10=>action_code-delete ) )
                           IMPORTING et_messages            = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF /dmo/cx_flight_legacy10 ) ).
    DATA lx TYPE REF TO /dmo/cx_flight_legacy10.
  ...
  METHOD u_booking_suppl_id_unknown.
    gr_cut->update_travel( EXPORTING is_travel              = VALUE #( travel_id = mv_travel_id )
                                     ...
                                     it_booking_supplementx = VALUE #( ( booking_id = mv_booking_id  booking_supplement_id = '10'  action_code = /dmo/if_flight_legacy10=>action_code-update  price = abap_true ) )
                           IMPORTING et_messages            = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF /dmo/cx_flight_legacy10 ) ).
    DATA lx TYPE REF TO /dmo/cx_flight_legacy10.
    lx ?= lt_messages[ 1 ].
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = /dmo/cx_flight_legacy10=>booking_supplement_unknown-msgno ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr1  exp = 'MV_TRAVEL_ID' ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr2  exp = 'MV_BOOKING_ID' ).
  ...
  METHOD u_booking_suppl_id_initial.
    gr_cut->update_travel( EXPORTING is_travel              = VALUE #( travel_id = mv_travel_id )
                                     ...
                                     it_booking_supplementx = VALUE #( ( booking_id = mv_booking_id  action_code = /dmo/if_flight_legacy10=>action_code-update  price = abap_true ) )
                           IMPORTING et_messages            = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF /dmo/cx_flight_legacy10 ) ).
    DATA lx TYPE REF TO /dmo/cx_flight_legacy10.
    lx ?= lt_messages[ 1 ].
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = /dmo/cx_flight_legacy10=>booking_supplement_no_key-msgno ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr1  exp = 'MV_TRAVEL_ID' ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr2  exp = 'MV_BOOKING_ID' ).
    cl_abap_unit_assert=>assert_equals( act = lx->mv_travel_id                   exp = mv_travel_id ).
    cl_abap_unit_assert=>assert_equals( act = lx->mv_booking_id                  exp = mv_booking_id ).
  ...
  METHOD d_booking_suppl_id_initial.
    gr_cut->update_travel( EXPORTING is_travel              = VALUE #( travel_id = mv_travel_id )
                                     ...
                                     it_booking_supplementx = VALUE #( ( booking_id = mv_booking_id  action_code = /dmo/if_flight_legacy10=>action_code-delete ) )
                           IMPORTING et_messages            = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF /dmo/cx_flight_legacy10 ) ).
    DATA lx TYPE REF TO /dmo/cx_flight_legacy10.
  ...
  METHOD c_bsuppl_d_booking_1_luw.
    ...
    CLEAR lv_db_exists.
    SELECT SINGLE FROM /dmo/book_sup_10 FIELDS @abap_true WHERE travel_id             = @ls_booking_supplement-travel_id
  ...
  METHOD d_booking_c_bsuppl_1_luw.
    ...
    gr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = mv_travel_id )
                                     ...
                                     it_bookingx = VALUE #( ( booking_id = mv_booking_id  action_code = /dmo/if_flight_legacy10=>action_code-delete ) )
                           IMPORTING et_messages = DATA(lt_messages) ).
    cl_abap_unit_assert=>assert_initial( lt_messages ).
    gr_cut->update_travel( EXPORTING is_travel              = VALUE #( travel_id = mv_travel_id )
                                     is_travelx             = VALUE #( travel_id = mv_travel_id )
  ...
  METHOD du_single.
    ...
    CLEAR lv_db_exists.
    SELECT SINGLE FROM /dmo/book_sup_10 FIELDS @abap_true WHERE travel_id = @mv_travel_id AND booking_id = @mv_booking_id AND booking_supplement_id = '22' INTO @lv_db_exists.
    cl_abap_unit_assert=>assert_true( lv_db_exists ).
  ...
  METHOD dd_single.
    CONSTANTS lc_booking_supplement_id TYPE /dmo/booking_supplement_id10 VALUE '34'.
    DATA lv_db_exists TYPE abap_bool.
    ...
    " Check existence of Booking Supplement
    CLEAR lv_db_exists.
    SELECT SINGLE FROM /dmo/book_sup_10 FIELDS @abap_true WHERE travel_id = @mv_travel_id AND booking_id = @mv_booking_id AND booking_supplement_id = @lc_booking_supplement_id INTO @lv_db_exists.
    cl_abap_unit_assert=>assert_true( lv_db_exists ).
  ...
  METHOD c_supplement_unknown.
    ...
    lx ?= lt_messages[ 1 ].
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = /dmo/cx_flight_legacy10=>supplement_unknown-msgno ).
    ...
    CLEAR lv_db_exists.
    SELECT SINGLE FROM /dmo/book_sup_10 FIELDS @abap_true WHERE travel_id  = @mv_travel_id AND booking_id = @mv_booking_id INTO @lv_db_exists.
    cl_abap_unit_assert=>assert_false( lv_db_exists ).
  ENDMETHOD.
  ...
  METHOD c_booking_suppl_id_exists.
    ...
    lx ?= lt_messages[ 1 ].
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = /dmo/cx_flight_legacy10=>booking_supplement_exists-msgno ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr1  exp = 'MV_TRAVEL_ID' ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr2  exp = 'MV_BOOKING_ID' ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr3  exp = 'MV_BOOKING_SUPPLEMENT_ID' ).
    cl_abap_unit_assert=>assert_equals( act = lx->mv_travel_id                   exp = mv_travel_id ).
  ...
  METHOD c_deep_insert.
    CONSTANTS lc_booking_id_1 TYPE /dmo/booking_id10 VALUE '20'.
    CONSTANTS lc_booking_id_2 TYPE /dmo/booking_id10 VALUE '21'.
    CONSTANTS lc_booking_supplement_id_1_1 TYPE /dmo/booking_supplement_id10 VALUE '10'.
    CONSTANTS lc_booking_supplement_id_2_1 TYPE /dmo/booking_supplement_id10 VALUE '10'.
    CONSTANTS lc_booking_supplement_id_2_2 TYPE /dmo/booking_supplement_id10 VALUE '20'.
    ...
    DATA ls_travel_sel TYPE /dmo/travel10.
    SELECT SINGLE agency_id, customer_id, total_price, currency_code, description FROM /dmo/travel10 WHERE travel_id = @ls_travel-travel_id INTO CORRESPONDING FIELDS OF @ls_travel_sel.
    cl_abap_unit_assert=>assert_subrc( ).
    ...
    SELECT COUNT( * ) FROM /dmo/booking10 WHERE travel_id = @ls_travel-travel_id INTO @DATA(lv_count).
    cl_abap_unit_assert=>assert_equals( act = lv_count  exp = 2 ).
    DATA ls_booking_sel TYPE /dmo/booking10.
    SELECT SINGLE customer_id, flight_date FROM /dmo/booking10 WHERE travel_id  = @ls_travel-travel_id
    ...
    cl_abap_unit_assert=>assert_equals( act = lv_count  exp = 3 ).
    DATA ls_book_suppl_sel TYPE /dmo/book_sup_10.
    SELECT SINGLE price, currency_code FROM /dmo/book_sup_10 WHERE travel_id             = @ls_travel-travel_id
                                                              AND booking_id            = @lc_booking_id_2
  ...
  METHOD u_single_travel_multiple.
    ...
    CONSTANTS lc_booking_id_1 TYPE /dmo/booking_id10 VALUE '20'.
    CONSTANTS lc_booking_supplement_id_1_1 TYPE /dmo/booking_supplement_id10 VALUE '10'.
    CONSTANTS lc_booking_supplement_id_1_2 TYPE /dmo/booking_supplement_id10 VALUE '20'.
    CONSTANTS lc_booking_supplement_id_1_3 TYPE /dmo/booking_supplement_id10 VALUE '30'.
    gr_cut->create_travel(
      EXPORTING
        is_travel             = VALUE #( agency_id     = gv_agency_id_1
                                         customer_id   = gv_customer_id_1
    ...
    CLEAR lv_db_exists.
    SELECT SINGLE FROM /dmo/book_sup_10 FIELDS @abap_true WHERE travel_id             = @ls_travel-travel_id
    ...
    CONSTANTS lc_booking_id_2 TYPE /dmo/booking_id10 VALUE '10'.
    CONSTANTS lc_booking_supplement_id_2_1 TYPE /dmo/booking_supplement_id10 VALUE '10'.
    CONSTANTS lc_booking_supplement_id_1_4 TYPE /dmo/booking_supplement_id10 VALUE '40'.
    CONSTANTS lc_diff TYPE /dmo/supplement_price10 VALUE '123.00'.
      ...
      EXPORTING
        ...
        it_booking             = VALUE #( ( booking_id = lc_booking_id_1  customer_id = gv_customer_id_2 )
                                          ( booking_id    = lc_booking_id_2
                                            ...
                                            flight_price  = '200.00'
                                            currency_code = 'EUR' ) )
        it_bookingx            = VALUE #( ( booking_id = lc_booking_id_1  action_code = /dmo/if_flight_legacy10=>action_code-update  customer_id = abap_true )
                                          ( booking_id = lc_booking_id_2  action_code = /dmo/if_flight_legacy10=>action_code-create ) )
        it_booking_supplement  = VALUE #( ( booking_id            = lc_booking_id_1
                                            booking_supplement_id = lc_booking_supplement_id_1_1
                                            price                 = lv_new_price
                                            currency_code         = 'EUR' )
    ...
    " Check Travel
    DATA ls_travel_sel TYPE /dmo/travel10.
    SELECT SINGLE total_price, currency_code, description FROM /dmo/travel10 WHERE travel_id = @ls_travel-travel_id INTO CORRESPONDING FIELDS OF @ls_travel_sel.
    cl_abap_unit_assert=>assert_subrc( ).
    ...
    " -- Updated Booking
    DATA ls_booking_sel TYPE /dmo/booking10.
    SELECT SINGLE booking_date, customer_id FROM /dmo/booking10 WHERE travel_id = @ls_travel-travel_id AND booking_id = @lc_booking_id_1 INTO CORRESPONDING FIELDS OF @ls_booking_sel.
    cl_abap_unit_assert=>assert_subrc( ).
    ...
    " -- New Booking
    CLEAR ls_booking_sel.
    SELECT SINGLE customer_id, flight_date FROM /dmo/booking10 WHERE travel_id = @ls_travel-travel_id AND booking_id = @lc_booking_id_2 INTO CORRESPONDING FIELDS OF @ls_booking_sel.
    cl_abap_unit_assert=>assert_subrc( ).
    ...
    DATA ls_booking_supplement_sel TYPE /dmo/book_sup_10.
    SELECT SINGLE supplement_id, price FROM /dmo/book_sup_10 WHERE travel_id             = @ls_travel-travel_id
                                                              AND booking_id            = @lc_booking_id_1
    ...
    SELECT SINGLE supplement_id FROM /dmo/book_sup_10 WHERE travel_id             = @ls_travel-travel_id
                                                       AND booking_id            = @lc_booking_id_1
  ...
  METHOD read_buffer.
    ...
    gr_cut->update_travel( EXPORTING is_travel   = VALUE #( travel_id = ls_travel-travel_id )
                                     is_travelx  = VALUE #( travel_id = ls_travel-travel_id )
                                     it_booking  = VALUE #( ( booking_id = lc_booking_id_1 ) )
                                     it_bookingx = VALUE #( ( booking_id = lc_booking_id_1  action_code = /dmo/if_flight_legacy10=>action_code-delete ) )
                           IMPORTING et_messages = lt_messages ).
    cl_abap_unit_assert=>assert_initial( lt_messages ).
  ...
  METHOD u_supplement_id.
    ...
    lx ?= lt_messages[ 1 ].
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = /dmo/cx_flight_legacy10=>booking_supplement_suppl_id_u-msgno ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr1  exp = 'MV_TRAVEL_ID' ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr2  exp = 'MV_BOOKING_ID' ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr3  exp = 'MV_BOOKING_SUPPLEMENT_ID' ).
    cl_abap_unit_assert=>assert_equals( act = lx->mv_travel_id                   exp = ls_booking_supplement-travel_id ).
    ...
    SELECT SINGLE supplement_id, price FROM /dmo/book_sup_10 WHERE travel_id             = @ls_booking_supplement-travel_id
                                                              AND booking_id            = @ls_booking_supplement-booking_id
                                                              AND booking_supplement_id = @ls_booking_supplement-booking_supplement_id
  ...
  METHOD c_price_currency.
    ...
    " Provide price and currency
    CONSTANTS lc_diff TYPE /dmo/supplement_price10 VALUE '9.99'.
    DATA lv_price         TYPE /dmo/supplement_price10.
    DATA lv_currency_code TYPE /dmo/currency_code10.
    ...
    DATA ls_booking_supplement_sel TYPE /dmo/book_sup_10.
    SELECT SINGLE price, currency_code FROM /dmo/book_sup_10 WHERE travel_id = @mv_travel_id AND booking_id = @mv_booking_id AND booking_supplement_id = '10' INTO CORRESPONDING FIELDS OF @ls_booking_supplement_sel.
    cl_abap_unit_assert=>assert_subrc( ).
    ...
    CLEAR ls_booking_supplement_sel.
    SELECT SINGLE price, currency_code FROM /dmo/book_sup_10 WHERE travel_id = @mv_travel_id AND booking_id = @mv_booking_id AND booking_supplement_id = '20' INTO CORRESPONDING FIELDS OF @ls_booking_supplement_sel.
    cl_abap_unit_assert=>assert_subrc( ).
  ...
  METHOD u_price_currency.
    ...
    lx ?= lt_messages[ 1 ].
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = /dmo/cx_flight_legacy10=>booking_supplement_pri_curr_u-msgno ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr1  exp = 'MV_TRAVEL_ID' ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr2  exp = 'MV_BOOKING_ID' ).
    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr3  exp = 'MV_BOOKING_SUPPLEMENT_ID' ).
    cl_abap_unit_assert=>assert_equals( act = lx->mv_travel_id                   exp = ls_booking_supplement-travel_id ).
