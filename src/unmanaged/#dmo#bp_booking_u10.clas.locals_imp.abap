...
CLASS lcl_message_helper DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    TYPES tt_booking_failed   TYPE TABLE FOR FAILED   /dmo/i_booking_u10.
    TYPES tt_booking_reported TYPE TABLE FOR REPORTED /dmo/i_booking_u10.

    CLASS-METHODS handle_booking_messages
      IMPORTING iv_cid        TYPE string OPTIONAL
                iv_travel_id  TYPE /dmo/travel_id10 OPTIONAL
                iv_booking_id TYPE /dmo/booking_id10 OPTIONAL
                it_messages   TYPE /dmo/if_flight_legacy10=>tt_message
      CHANGING
...
CLASS lcl_message_helper IMPLEMENTATION.
  ...
  METHOD handle_booking_messages.
    ...
    LOOP AT it_messages INTO DATA(ls_message) WHERE msgty = 'E' OR msgty = 'A'.
      ...
      INSERT /dmo/cl_travel_auxiliary10=>map_booking_message(
...
CLASS lhc_booking DEFINITION INHERITING FROM cl_abap_behavior_handler.
  ...
  PRIVATE SECTION.
    ...
    TYPES:
        tt_booking_update           TYPE TABLE FOR UPDATE  /dmo/i_booking_u10,
        tt_bookingsupplement_create TYPE TABLE FOR CREATE  /dmo/i_bookingsupplement_u10.
    ...
    METHODS:
        _fill_booking_inx
                ...
                RETURNING VALUE(rs_booking_inx) TYPE /dmo/if_flight_legacy10=>ts_booking_inx,
        _fill_bookingsupplement_inx
                IMPORTING is_bookingsupplement_create     TYPE LINE OF tt_bookingsupplement_create
                RETURNING VALUE(rs_bookingsupplement_inx) TYPE /dmo/if_flight_legacy10=>ts_booking_supplement_inx.
...
**********************************************************************
  METHOD update_booking.
    ...
    DATA lt_messages TYPE /dmo/if_flight_legacy10=>tt_message.

    LOOP AT it_booking_update ASSIGNING FIELD-SYMBOL(<fs_booking_update>).
      CALL FUNCTION '/DMO/FLIGHT_TRAVEL_UPDATE10'
        EXPORTING
          is_travel   = VALUE /dmo/if_flight_legacy10=>ts_travel_in( travel_id = <fs_booking_update>-travelid )
          is_travelx  = VALUE /dmo/if_flight_legacy10=>ts_travel_inx( travel_id = <fs_booking_update>-travelid )
          it_booking  = VALUE /dmo/if_flight_legacy10=>tt_booking_in( ( /dmo/cl_travel_auxiliary10=>map_booking_cds_to_db( CORRESPONDING #( <fs_booking_update> ) ) ) )
          it_bookingx = VALUE /dmo/if_flight_legacy10=>tt_booking_inx( ( _fill_booking_inx( <fs_booking_update> ) ) )
        IMPORTING
...
**********************************************************************
  METHOD _fill_booking_inx.
    ...
    rs_booking_inx-action_code   = /dmo/if_flight_legacy10=>action_code-update.
...
**********************************************************************
  METHOD delete_booking.
    ...
    LOOP AT it_booking_delete INTO DATA(ls_booking_delete).
      ...
      CALL FUNCTION '/DMO/FLIGHT_TRAVEL_UPDATE10'
        EXPORTING
          is_travel   = VALUE /dmo/if_flight_legacy10=>ts_travel_in( travel_id = ls_booking_delete-travelid )
          is_travelx  = VALUE /dmo/if_flight_legacy10=>ts_travel_inx( travel_id = ls_booking_delete-travelid )
          it_booking  = VALUE /dmo/if_flight_legacy10=>tt_booking_in( ( booking_id = ls_booking_delete-bookingid ) )
          it_bookingx = VALUE /dmo/if_flight_legacy10=>tt_booking_inx( ( booking_id  = ls_booking_delete-bookingid
...
***********************************************************************
  METHOD cba_supplement.
    ...
    DATA lt_messages               TYPE /dmo/if_flight_legacy10=>tt_message.
    DATA lt_booksupplement_old     TYPE /dmo/if_flight_legacy10=>tt_booking_supplement.
    DATA ls_booksupplement         TYPE LINE OF /dmo/if_flight_legacy10=>tt_booking_supplement_in.
    DATA lv_last_booksupplement_id TYPE /dmo/booking_supplement_id10.
    ...
    LOOP AT it_supplement_create_ba ASSIGNING FIELD-SYMBOL(<fs_supplement_create_ba>).
      ...
      CALL FUNCTION '/DMO/FLIGHT_TRAVEL_READ10'
        EXPORTING
      ...
      IF lt_messages IS INITIAL.
        ...
        LOOP AT <fs_supplement_create_ba>-%target INTO DATA(ls_supplement_create).
          ...
          ls_booksupplement = /dmo/cl_travel_auxiliary10=>map_bookingsupplemnt_cds_to_db( CORRESPONDING #( ls_supplement_create ) ) .

          " Create a new booking supplement and update a booking instance
          CALL FUNCTION '/DMO/FLIGHT_TRAVEL_UPDATE10'
            EXPORTING
              is_travel              = VALUE /dmo/if_flight_legacy10=>ts_travel_in( travel_id = ls_parent_key-travelid )
              is_travelx             = VALUE /dmo/if_flight_legacy10=>ts_travel_inx( travel_id = ls_parent_key-travelid )
              it_bookingx            = VALUE /dmo/if_flight_legacy10=>tt_booking_inx( ( booking_id  = ls_parent_key-bookingid
                                                                                      action_code = /dmo/if_flight_legacy10=>action_code-update ) )
              it_booking_supplement  = VALUE /dmo/if_flight_legacy10=>tt_booking_supplement_in( ( ls_booksupplement ) )
              it_booking_supplementx = VALUE /dmo/if_flight_legacy10=>tt_booking_supplement_inx( ( _fill_bookingsupplement_inx( ls_supplement_create ) ) )
            IMPORTING
              et_messages = lt_messages.
          ...
          ELSE.
            ...
            LOOP AT lt_messages INTO DATA(ls_message) WHERE msgty = 'E' OR msgty = 'A'.
              ...
              INSERT /dmo/cl_travel_auxiliary10=>map_bookingsupplemnt_message(
...
**********************************************************************
  METHOD _fill_bookingsupplement_inx.
    ...
    rs_bookingsupplement_inx-action_code           = /dmo/if_flight_legacy10=>action_code-create.
