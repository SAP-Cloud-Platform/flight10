...
CLASS lcl_message_helper DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    TYPES tt_travel_failed      TYPE TABLE FOR FAILED   /dmo/i_travel_u10.
    TYPES tt_travel_reported    TYPE TABLE FOR REPORTED /dmo/i_travel_u10.
  ...
  METHOD handle_travel_messages.
    LOOP AT it_messages INTO DATA(ls_message) WHERE msgty = 'E' OR msgty = 'A'.
      INSERT VALUE #( %cid = iv_cid  travelid = iv_travel_id )
             INTO TABLE failed.
      INSERT /dmo/cl_travel_auxiliary10=>map_travel_message(
...
**********************************************************************
  METHOD create_travel.
    ...
    DATA ls_travel_in  TYPE /dmo/if_flight_legacy10=>ts_travel_in.
    DATA ls_travel_out TYPE /dmo/travel10.

    LOOP AT it_travel_create ASSIGNING FIELD-SYMBOL(<fs_travel_create>).
      CLEAR ls_travel_in.
      ls_travel_in = CORRESPONDING #( /dmo/cl_travel_auxiliary10=>map_travel_cds_to_db( CORRESPONDING #( <fs_travel_create> ) ) ).

      CALL FUNCTION '/DMO/FLIGHT_TRAVEL_CREATE10'
        EXPORTING
...
**********************************************************************
  METHOD update_travel.
    ...
    DATA lt_messages    TYPE /dmo/if_flight_legacy10=>tt_message.
    DATA ls_travel      TYPE /dmo/if_flight_legacy10=>ts_travel_in.
    DATA ls_travelx TYPE /dmo/if_flight_legacy10=>ts_travel_inx. "refers to x structure (> BAPIs)

    LOOP AT it_travel_update ASSIGNING FIELD-SYMBOL(<fs_travel_update>).

      CLEAR ls_travel.
      ls_travel = CORRESPONDING #( /dmo/cl_travel_auxiliary10=>map_travel_cds_to_db( CORRESPONDING #( <fs_travel_update> ) ) ).
...
**********************************************************************
  METHOD delete_travel.
    ...
    DATA lt_messages TYPE /dmo/if_flight_legacy10=>tt_message.
    DATA ls_travel   TYPE /dmo/if_flight_legacy10=>ts_travel_key.
    ...
    LOOP AT it_travel_delete ASSIGNING FIELD-SYMBOL(<fs_travel_delete>).
      ...
      CALL FUNCTION '/DMO/FLIGHT_TRAVEL_DELETE10'
        EXPORTING
...
**********************************************************************
  ...
  METHOD set_travel_status.
    ...
    LOOP AT it_travel_set_status_booked ASSIGNING FIELD-SYMBOL(<fs_travel_set_status_booked>).
      ...
      CALL FUNCTION '/DMO/FLIGHT_TRAVEL_SET_BOOK_10'
        EXPORTING
...
**********************************************************************
  METHOD cba_booking.
    DATA lt_messages        TYPE /dmo/if_flight_legacy10=>tt_message.
    DATA lt_booking_old     TYPE /dmo/if_flight_legacy10=>tt_booking.
    DATA ls_booking         TYPE LINE OF /dmo/if_flight_legacy10=>tt_booking_in.
    DATA lv_last_booking_id TYPE /dmo/booking_id10 VALUE '0'.
    ...
    LOOP AT it_booking_create_ba ASSIGNING FIELD-SYMBOL(<fs_booking_create_ba>).
      ...
      IF lt_messages IS INITIAL.
        ...
        LOOP AT <fs_booking_create_ba>-%target ASSIGNING FIELD-SYMBOL(<fs_booking_create>).
          ls_booking = /dmo/cl_travel_auxiliary10=>map_booking_cds_to_db( CORRESPONDING #( <fs_booking_create> ) ).
          ...
          CALL FUNCTION '/DMO/FLIGHT_TRAVEL_UPDATE10'
            EXPORTING
              is_travel   = VALUE /dmo/if_flight_legacy10=>ts_travel_in( travel_id = lv_travelid )
              is_travelx  = VALUE /dmo/if_flight_legacy10=>ts_travel_inx( travel_id = lv_travelid )
              it_booking  = VALUE /dmo/if_flight_legacy10=>tt_booking_in( ( ls_booking ) )
              it_bookingx = VALUE /dmo/if_flight_legacy10=>tt_booking_inx( ( booking_id  = ls_booking-booking_id
...
CLASS lsc_saver IMPLEMENTATION.
  ...
  METHOD save.
    CALL FUNCTION '/DMO/FLIGHT_TRAVEL_SAVE10'.
  ENDMETHOD.

  METHOD cleanup.
    CALL FUNCTION '/DMO/FLIGHT_TRAVEL_INITIALI_10'.
  ENDMETHOD.
ENDCLASS.
