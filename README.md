...
1. In your ABAP cloud project, create the ABAP package `/DMO/FLIGHT10` (using the superpackage `/DMO/SAP`) as the target package for the demo content to be downloaded (leave the suggested values unchanged when following the steps in the package creation wizard).
...
5. Select the <em>master</em> branch and enter the newly created package `/DMO/FLIGHT10` as the target package.
...
* `/DMO/FLIGHT_LEGACY`
* `/DMO/FLIGHT_R10EUSE`
* `/DMO/FLIGHT_R10EADONLY` - represents a read-only list reporting app (see also: corresponding [end-to-end guide](https://help.sap.com/viewer/c0d02c4330c34b3abca88bdd57eaccfc/Cloud/en-US/a1243bff462b4ee3a03e2bb6fc30e015.html)).
...
## Configuration
To activate all development objects from the `/DMO/FLIGHT10` package: 
...
To create a <em>service binding</em> for the read-only list reporting app (package `/DMO/FLIGHT_R10EADONLY`):
1. Right-click the service definition `/DMO/FLIGHT_R10` and choose `New Service Binding` (see [here](https://help.sap.com/viewer/c0d02c4330c34b3abca88bdd57eaccfc/Cloud/en-US/6709cab6cb5b4c01b28463d760429a9a.html) for additional information). 
...
1. Right-click the service definition `/DMO/TRAVEL_U10` and choose `New Service Binding` (see [here](https://help.sap.com/viewer/c0d02c4330c34b3abca88bdd57eaccfc/Cloud/en-US/9cda72a7bd9e476b8696f625da404605.html) for additional information). 
...
2. Select the data generator class `/DMO/CL_FLIGHT_DATA_GENERAT_10` and press `F9` (Run as Console Application). 
